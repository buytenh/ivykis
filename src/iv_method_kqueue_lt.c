...
static int iv_kqueue_lt_init(int maxfd)
{
	kqueue_fd = kqueue();
	if (kqueue_fd < 0)
		return -1;
...
static void iv_kqueue_lt_poll(int msec)
{
	...
	if (ret < 0) {
		syslog(LOG_CRIT, "iv_kqueue_lt_poll: got error %d[%s]",
		       errno, strerror(errno));
		abort();
	}
	...
	for (i = 0; i < ret; i++) {
		...
		if (batch[i].filter == EVFILT_READ) {
			...
			syslog(LOG_CRIT, "iv_kqueue_lt_poll: got message from "
...
static void iv_kqueue_lt_deinit(void)
{
	free(upload_queue);
	free(batch);
	close(kqueue_fd);
